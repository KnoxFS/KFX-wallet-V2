...
*Setup instructions for a gitian build of KFX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KFX
...
To build KFX (for Linux, OSX and Windows) just follow the steps under 'perform
...
```bash
    ...
    From https://github.com/knoxfs/kfx-core
...
For example:
```bash
URL=https://github.com/knoxfs/kfx-core.git
...
[Harcomia-KFX/gitian.sigs](https://github.com/mongo-cm-KFX/gitian.sigs/) repository, or if that's not possible to create a pull
request.
