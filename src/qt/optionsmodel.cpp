...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeKFXAmount"))
        settings.setValue("nAnonymizeKFXAmount", 1000);

    nAnonymizeKFXAmount = settings.value("nAnonymizeKFXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKFXAmount"))
        SoftSetArg("-anonymizekfxamount", settings.value("nAnonymizeKFXAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKFXAmount:
            return QVariant(nAnonymizeKFXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKFXAmount:
            nAnonymizeKFXAmount = value.toInt();
            settings.setValue("nAnonymizeKFXAmount", nAnonymizeKFXAmount);
            emit anonymizeKFXAmountChanged(nAnonymizeKFXAmount);
            break;
        case CoinControlFeatures:
