...
#include "zkfxcontroldialog.h"
#include "ui_zkfxcontroldialog.h"
...
std::list<std::string> ZKFXControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZKFXControlDialog::listMints;

ZKFXControlDialog::ZKFXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZKFXControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZKFXControlDialog::~ZKFXControlDialog()
{
    delete ui;
}

void ZKFXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKFXControlDialog::updateList()
{
...
void ZKFXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKFXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZKFX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));
}

std::vector<CZerocoinMint> ZKFXControlDialog::GetSelectedMints()
{
...
void ZKFXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
