...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, 	// ONLY_NONDENOMINATED and not 1000 KFX at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                      // UTXO's that are valid for staking
};

// Possible states for zKFX send
enum ZerocoinSpendStatus {
    ZKFX_SPEND_OKAY = 0,                            // No error
    ZKFX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZKFX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZKFX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZKFX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZKFX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZKFX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZKFX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZKFX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZKFX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZKFX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZKFX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZKFX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZKFX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZKFX_SPENT_USED_ZKFX = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZKFXAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000 KFX output and keys which can be used for the Masternode
