...
#if defined(NDEBUG)
#error "KFX cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zKFX spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zKFX spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zKFX spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 0.63 * COIN;
    } else if (nHeight < 540001) {
        nSubsidy = 0.66 * COIN;
    } else if (nHeight < 600001) {
        nSubsidy = 0.69 * COIN;
    } else if (nHeight < 660001) {
        nSubsidy = 0.72 * COIN;
    } else if (nHeight < 720001) {
        nSubsidy = 0.76 * COIN;
    } else if (nHeight < 780001) {
        nSubsidy = 0.79 * COIN;
    } else if (nHeight < 840001) {
        nSubsidy = 0.82 * COIN;
    } else if (nHeight < 900001) {
        nSubsidy = 0.85 * COIN;
    } else if (nHeight < 960001) {
        nSubsidy = 0.88 * COIN;
    } else if (nHeight < 1020001) {
        nSubsidy = 0.92 * COIN;
    } else if (nHeight < 1080001) {
        nSubsidy = 0.95 * COIN;
    } else if (nHeight < 1140001) {
        nSubsidy = 0.98 * COIN;
    } else if (nHeight < 1200001) {
        nSubsidy = 1.01 * COIN;
    } else if (nHeight < 1260001) {
        nSubsidy = 1.04 * COIN;
    } else if (nHeight < 1320001) {
        nSubsidy = 1.08 * COIN;
    } else if (nHeight < 1380001) {
        nSubsidy = 1.11 * COIN;
    } else if (nHeight < 1440001) {
        nSubsidy = 1.14 * COIN;
    } else if (nHeight < 1500001) {
        nSubsidy = 1.17 * COIN;
    } else if (nHeight < 1560001) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight < 1620001) {
        nSubsidy = 1.24 * COIN;
    } else if (nHeight < 1680001) {
        nSubsidy = 1.27 * COIN;
    } else if (nHeight < 1740001) {
        nSubsidy = 1.30 * COIN;
    } else if (nHeight < 3847679) {
        nSubsidy = 1.5 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from KFX
...
void RecalculateZKFXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zKFX supply
...
bool RecalculateKFXSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zKFX with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zKFXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert KFX to zKFX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zKFX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zKFX serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> kfxInputs;
          std::vector<CTxIn> zKFXInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zKFXInputs.push_back(stakeIn);
              }else{
                  kfxInputs.push_back(stakeIn);
              }
          }
          const bool hasKFXInputs = !kfxInputs.empty();
          const bool hasZKFXInputs = !zKFXInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasKFXInputs)
                      ...
                      for (CTxIn kfxIn : kfxInputs){
                          if(kfxIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zKFX inputs.
              if(hasZKFXInputs){
                  for (CTxIn zPawsInput : zKFXInputs) {
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zkfx");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zKFX mints and %d zKFX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KFX: We use certain sporks during IBD, so check to see if they are
